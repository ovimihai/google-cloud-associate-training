...
- Local SSD 
    ...
    - physically attached to the physical hardware
...
- manage patches from a centralized interface
...
- General purpose
    ...
    - E2 - cost -optimized day to day computing
        - lowest price especially with committed-use discounts
        - 2 to 32 CPUs
...
- Compute-optimized
    ...
    - C2D - largest VM sizes
        ...
        - 3rd Generation AMD Epic Millan
- Memory-optimized
    - lowest cost per GM memory
    - for tasks that require higher memory-to-vCPU rations
    - 30% sustained use discounts, 60% for three-year commitments
    - M1 - up to 4TB mem
        - good for: medium in-memory database like SAP HANA, in-memory databases and analytics, MS SQL Servers and similar DBs, business warehousing
    - M2 - up to 12TB
...
- Discounts - can't be combined
    ...
    - committed use (1-3y) - up to 57% for most tires, up to 70% for memory-optimized
...
- good for
    - payment processing that needs to be isolated for compliance requirements
...
- VMs comes with a single root persistent disk loaded with a bootable image 
    - durable image - survives termination
...
- can be dynamically resized while running
...
- Zonal or Regional
    ...
    - pd-balanced - on SSDs
    - pd-extreme - for high-end database workloads, you can specify desired IOPS
- Encryption keys:
    - Google-managed
    - Customer-managed
    - Customer-supplied
- Local SSDs - physically attached to a VM
    - ephemeral
    - more IOPS
...
- Maximum persistent disks
    - 16 for Shared-core
    - 128 for the rest
- Throughput shares the same bandwidth as Disk I/O - remember that if you need a large amount of persistent disks
...
- Move VM
    - Automated process (within region)
    ...
    - Manual process (between regions)
        ...
        - create new persistent disks in destimation zone restored from snapshots
...
- can be used to transfer from HDD to SSD
